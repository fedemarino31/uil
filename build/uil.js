...
(function (global, factory) {
	...
	class TreeList extends Proto {
		...
		paintItemDom(div, L, i, it, kind, isReal = true) {
			...
			const isHover = this.enableHover && isReal && this.hover.level === L && this.hover.index === i;
		...
		handleEvent(e) {
			...
			if (e.type === "mousemove") {
				...
				if (newHover.level === this.hover.level && newHover.index === this.hover.index) return false;
				this.hover = newHover;
				this.update();
				return true;
			}
	...
	class Tab extends Proto {
		...
		handleEvent(e) {
			...
			switch (name) {
				case 'content':
					if (Roots.isMobile && type === 'mousedown') this.getNext(e, change);else if (type === 'mousedown' || type === 'pointerdown' || type === 'touchstart') this.getNext(e, change);
		...
		calcUis() {
			if (!this.isOpen || this.isEmpty) this.h = 0;else this.h = Roots.calcUis([...this.uis, this.spaceY], this.zone, this.zone.y + this.margin, true);
			this.zone.h = this.h;
		}
		rSizeContent() {
			let i = this.uis.length;
	...
	class Gui {
		...
		ensureTabBar() {
			...
			const cc = this.colors;
			this.tabBarWrapper = Tools.dom("div", this.css.basic + "position:relative; width:100%; left:0; top:0; height:auto; pointer-events:none;");
			...
			this.innerContent.insertBefore(this.tabBarWrapper, this.innerContent.firstChild);
			this.tabBar = Tools.dom("div", this.css.basic + "position:relative; width:100%; left:0; top:0; height:auto; pointer-events:auto; display:flex; flex-wrap:wrap; align-items:stretch;", null, this.tabBarWrapper);
			this.tabBar.style.margin = "0";
			this.tabBar.style.padding = "0";
			this.tabBar.style.background = cc.background;
			this.tabBar.style.borderBottom = cc.borderSize + "px solid " + cc.border;
		}
		registerTab(tab) {
			this.ensureTabBar();
			if (this.tabs.indexOf(tab) === -1) this.tabs.push(tab);
			const cc = this.colors;
			const button = Tools.dom("div", this.css.txt + this.css.button + "position:relative; pointer-events:auto; cursor:pointer; height:" + this.size.h + "px; line-height:" + (this.size.h - 5) + "px; margin:2px 4px 0 0; padding:0px 10px; border:" + cc.borderSize + "px solid " + cc.border + "; border-bottom:none; border-top-left-radius:" + cc.radius + "px; border-top-right-radius:" + cc.radius + "px;");
		...
		unregisterTab(tab) {
			...
			if (tab.tabButton && this.tabBar && tab.tabButton.parentNode === this.tabBar) this.tabBar.removeChild(tab.tabButton);
			tab.tabButton = null;
		...
		updateTabButton(tab, active) {
			...
			button.style.borderBottom = active ? "none" : cc.borderSize + "px solid " + cc.border;
		}
		updateTabBarLayout() {
		...
		calcUis() {
			...
			const controlsHeight = Roots.calcUis(this.uis, this.zone, this.zone.y + nav);
