...
(function (global, factory) {
	...
	class Tab extends Proto {
		...
		createButton() {
			...
			const buttonCss = this.css.txt + this.css.button + 'position:relative; pointer-events:auto; cursor:pointer; width:auto; padding:0 12px; height:' + (h - 2) + 'px; line-height:' + (h - 4) + 'px; border:' + cc.borderSize + 'px solid ' + cc.border + '; border-radius:' + this.radius + 'px;';
		...
		calcUis() {
			if (!this.isActive || this.isEmpty) this.h = 0;else this.h = Roots.calcUis(this.uis, this.zone, this.zone.y, true);
			this.s[0].height = this.h + 'px';
			this.zone.h = this.h + this.margin;
		}
		...
		notifyParent() {
			if (this.group !== null) this.group.calc();else if (this.isUI && this.main) this.main.calc();
		}
		dispose() {
			this.removeButton();
	...
	class Gui {
		...
		ensureTabContainers() {
			...
			const flex = "display:flex; flex-flow: row wrap; gap:" + this.colors.sx + "px;";
			this.tabBar = Tools.dom("div", this.css.basic + flex + "width:100%; left:0; top:0; pointer-events:none;");
			...
			this.tabContent = Tools.dom("div", this.css.basic + "width:100%; left:0; top:0; pointer-events:none;");
			this.tabContent.style.display = "block";
			this.inner.insertBefore(this.tabBar, this.inner.firstChild);
			if (this.tabBar.nextSibling) this.inner.insertBefore(this.tabContent, this.tabBar.nextSibling);else this.inner.appendChild(this.tabContent);
		}
		removeTabContainers() {
